...
class TestChatEndpoint:
    ...
    def test_document_status_no_pdid(self, client):
        ...
        response = client.post("/ai/chat", json={"message": "What is the status of my document?"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_document_status_with_pdid(self, client):
        ...
        response = client.post("/ai/chat", json={"message": "Where is PDID 001?"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_multi_turn_conversation(self, client):
        ...
        r1 = client.post("/ai/chat", json={"message": "What is the status of my document?"})
        assert r1.status_code == 200
        session_id = r1.json()["session_id"]

        # Turn 2: Provide PDID
        r2 = client.post("/ai/chat", json={
            "message": "PDID 001",
    ...
    def test_unknown_pdid(self, client):
        ...
        response = client.post("/ai/chat", json={"message": "Where is PDID 999?"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_help_intent(self, client):
        response = client.post("/ai/chat", json={"message": "How do I use this system?"})
        assert response.status_code == 200
        data = response.json()
        assert "reply" in data
...
class TestTrainEndpoint:
    def test_train_from_csv(self, client):
        response = client.post("/ai/train", json={"source": "csv"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_train_invalid_source(self, client):
        response = client.post("/ai/train", json={"source": "invalid"})
